...
/**
 * Script para otimizar dados hidrográficos para uso web
 * - Simplifica geometrias para reduzir tamanho
 * - Cria versões por nível de zoom
 * - Gera arquivos de cache para frontend
 */
...
/**
 * Carrega dados base do manifesto
 */
function loadBaseData() {
  const manifestPath = path.join(BASE_DIR, 'manifest.json');
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Manifest não encontrado. Execute create-sample-data.js primeiro.');
  }
  
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  console.log(`📋 Carregando dados: ${manifest.source} v${manifest.version}`);
  
  const data = {};
  for (const territory of manifest.territories) {
    const filePath = path.join(BASE_DIR, `${territory.toLowerCase()}-rivers.geojson`);
    if (fs.existsSync(filePath)) {
      data[territory] = JSON.parse(fs.readFileSync(filePath, 'utf8'));
      console.log(`   ✓ ${territory}: ${data[territory].features.length} rios`);
    }
  }
...
function processTerritory(territory, geojson) {
  ...
  console.log(`   Rios originais: ${geojson.features.length}`);
...
function createGlobalManifest(territories) {
  ...
  console.log(`   Rios totais: ${globalManifest.statistics.totalRivers}`);
 ...
 */
async function main() {
  console.log('⚡ Otimizando dados hidrográficos para web');
